  ...
  def getDistanceOnGrid(self, pos1, pos2):
    ...
    if key in self._distances:
      return self._distances[key]
        ...
        if self.errorToF is False:
            frame = inspect.currentframe().f_back
            pframe = inspect.frame.f_back
            ppframe = inspect.pframe.f_back
            (filename, line_number, function_name, lines, index) = inspect.getframeinfo(ppframe)
            arr = filename.split(os.sep)
            ctypes.windll.user32.MessageBoxW(0, "Positions not in grid", arr[len(arr)-1] + "Disqualified", 1)
            self.errorToF = True

        print("method getDistanceOnGrid returns any number... ")
        return 100000
